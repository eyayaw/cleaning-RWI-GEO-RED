...
extract_dname = sprintf("%s/%s", dname, file_stem(zfpath)) # the extracted folder
...
st_write(de_grid, file.path(grid_dname, "de-grid.gpkg"), append = FALSE)
...
## districts shape
district_shape = st_read(sub("GEM", "KRS", shape_path)) |>
  {\(.x) setNames(.x, tolower(names(.x)))}() |>
  subset(gf == 4L, select = c("ags", "geometry")) |>
  setNames(c("did", "geometry")) |>
...
fwrite(municipals, file.path(dname, "municipalities_bkg.csv"))
fwrite(districts, file.path(dname, "districts_bkg.csv"))
fwrite(states, file.path(dname, "states.csv"))

## admin area shapes
st_write(municipality_shape, file.path(dname, "municipalities_bkg.gpkg"), append = FALSE)
st_write(district_shape, file.path(dname, "districts_bkg.gpkg"), append = FALSE)
