...
home_sales = fread(fpath) # KeepLeadingZeros = TRUE is not required, rwi data comes without the leading zeros
...
if (!("grid_id" %in% names(home_sales))) {
  if ("ergg_1km" %in% names(home_sales)) {
    setnames(home_sales, "ergg_1km", "grid_id")
  } else if ("grid_id_char" %in% names(home_sales)) {
    setnames(home_sales, "grid_id_char", "grid_id")
  }
}

setcolorder(home_sales, c(order_of_vars, setdiff(names(home_sales), order_of_vars)))
setnames(home_sales, 'kid2019', 'did') # rename kid2019 to `did` (district id)

# rm duplicates
n = nrow(home_sales)
idx = home_sales[, .(idx=.I[which.max(spell)]), .(obid, year)][, idx]
home_sales = home_sales[idx, ]
nrow(home_sales)/n
home_sales[, spell := NULL]
rm(idx)

## dealing with missing values of many forms -----------------------------------
home_sales = home_sales[zipcode>0, ]
### drop observations with missing district identifier ----
count_missing = list(did = home_sales[did < 0, .N])
home_sales = home_sales[did > 0, ]

### drop obs with missing 1km grid identifier, i.e., that are annonymized
count_missing$grid_id = home_sales[grid_id<0, .N]
home_sales = home_sales[grid_id > 0, ]

### drop rows if price is missing ----
count_missing$price = home_sales[price <= 0, .N]
home_sales = home_sales[price > 0, ]

### drop if missing floor space ----
count_missing$floor_space = home_sales[floor_space <= 0, .N]
home_sales = home_sales[floor_space > 0, ]

### plot area ----
# count_missing$plot_size = home_sales[plot_size <= 0, .N]
# home_sales = home_sales[plot_size > 0, ]

### number of rooms -----
count_missing$num_rooms = home_sales[num_rooms < 0, .N]
home_sales = home_sales[num_rooms > 0, ]

### house type ----
count_missing$house_type = home_sales[house_type < 0, .N]
home_sales[, house_type := fcase(
...
count_missing$condition = home_sales[condition < 0, .N]
home_sales[condition < 0, condition := 0L]
home_sales[, condition := factor(
  condition,
  0L:10L,
  c(
...
### number of bedrooms ----
count_missing$num_bedrooms = home_sales[num_bedrooms <= 0, .N]
home_sales[, num_bedrooms := fcase(
  num_bedrooms <= 0, 0L,
  num_bedrooms >= 7, 7L,
  rep_len(TRUE, length(num_bedrooms)), num_bedrooms
)]

home_sales[, num_bedrooms := factor(
  num_bedrooms,
  0:7,
  c("na or 0", 1:6, "7+")
)]
...
### number of bathrooms ----
count_missing$num_bathrooms = home_sales[num_bathrooms <= 0, .N]
home_sales[, num_bathrooms := fcase(
  num_bathrooms <= 0, 0L,
  num_bathrooms >= 4, 4L,
  rep_len(TRUE, length(num_bathrooms)), num_bathrooms
)]

home_sales[, num_bathrooms := factor(
  num_bathrooms,
  0:4,
  c("na or 0", 1:3, "4+")
)]
...
home_sales[, num_floors := fcase(
  between(num_floors, -11, 0), 0L,
  between(num_floors, 4, max(num_floors)), 4L,
  rep_len(TRUE, length(num_floors)), num_floors
)]

home_sales[, num_floors := factor(num_floors, 0:4, c("na", 1:3, "4+"))]

### facilities of the house, create categories ----
home_sales[between(equipment, -11, 0), equipment := 0]
home_sales[, equipment := factor(
  equipment,
  0:4,
...
home_sales[, c("constr_year_cat", "renov_year_cat") := lapply(.SD, function(x) {
  fcase(
    x <= 0, 0,
    x < 1900, 1,
...
home_sales[, c("constr_year_cat", "renov_year_cat") := lapply(.SD,
                                                         factor,
                                                         levels = 0:9,
                                                         labels = c(
...
### construction phase ----
home_sales[constr_phase <= 0, constr_phase := 0L]
home_sales[, constr_phase := factor(
  constr_phase,
  0L:3L,
...
for (i in seq_along(binary_vars)) {
  check_for_0[[i]] =
    all(0 %in% unique(home_sales[[binary_vars[[i]]]]))
}

for (i in seq_along(binary_vars)) {
  if (check_for_0[[i]]) {
    home_sales[, (binary_vars[[i]]) := lapply(.SD, function(v) {
      fcase(v == -9 | v == -7, 0L,
...
home_sales[, (binary_vars) := lapply(.SD, as.factor), .SDcols = binary_vars]

# house keeping
rm(order_of_vars, binary_vars, i, check_for_0)

# remove protected buildings
home_sales = home_sales[protected_building == 0, ]
home_sales[, protected_building := NULL]
...
home_sales = merge(home_sales, districts, 'did') # two districts c('3152', '3156') will be dropped

if (FALSE)
  message(sprintf('%.2f%% of the observations dropped.', 100 * home_sales[, .N]/fread(fpath)[, .N]))
...
grid1km = merge(grid1km, unique(home_sales[, .(grid_id, did)]), by="grid_id")
...
rm(dids)
home_sales = merge(home_sales, dist2cbd, c('grid_id', 'did'))
...
home_sales = merge(home_sales, cpi[year >= min(home_sales$year), ],
              by.x = c("year", "ad_end_mon"), by.y = c("year", "mon")
              )
home_sales[, price := price / (cpi/100)]  # divide by the deflator
home_sales[, cpi := NULL]


# zipcodes ----
home_sales[, `:=`(zipcode = sprintf('%05i', zipcode))] # make 5 digit

# Define new vars ----
home_sales[, c('lnprice', 'price_sqm') := .(log(price), price/floor_space)]
home_sales[, lnprice_sqm := log(price_sqm)]
setkeyv(home_sales, c("did", "zipcode", "ad_end_mon", "year"))

# optional: further cleaning for  hedonic model-----

# problematic construction and renovation years
home_sales[constr_year < 0, constr_year := NA][renov_year < 0, renov_year := NA]

# perhaps houses not finished or built yet
max_year = home_sales[, max(year)]
home_sales[constr_year > max_year, constr_year := (max_year)]
# If renovated before built, swap construction year with renovation year
home_sales[renov_year < constr_year,
      `:=`(renov_year = constr_year, constr_year = renov_year)]

# keep houses built since 1900
home_sales = home_sales[constr_year >= 1900 | is.na(constr_year), ]
...
home_sales[is.na(constr_year) & !is.na(renov_year), constr_year := renov_year]
home_sales[is.na(renov_year) &  !is.na(constr_year), renov_year := constr_year]
...
n = nrow(home_sales)
home_sales = home_sales[exp(lnprice_sqm) >= 250 & exp(lnprice_sqm) <= 25000 &
                            floor_space >= 30 & floor_space <= 500, ]

nrow(home_sales)/n # without duplicates

# write to disk ----
if (file.exists('data/processed/homes_ready.csv')) {
  warning("File has been overwritten!", call. = FALSE)
  fwrite(home_sales, 'data/processed/homes_ready.csv')
} else{
  fwrite(home_sales, 'data/processed/homes_ready.csv')
}
if (FALSE) haven::write_dta(home_sales, 'data/processed/homes_ready.dta')


## single family homes only ----
fwrite(home_sales[house_type == 'single-family', !'house_type'],
       'data/processed/homes_single-family_ready.csv')
if (FALSE) {
  home_sales[house_type == 'single-family', !'house_type'] |>
